...
class DisplayUnit:
    """An object which will display a value. It acts kind of like a register."""
    def __init__(self, start, width, height):
        self.start = start
        self.width = width
        self.height = height
        self.font = pygame.font.SysFont("ariel", 60)
        self.value = None
        return
    ...
    def draw(self, win):
        """Draws the display unit with it's value."""
        pygame.draw.rect(win, (183, 161, 230), (*self.start, self.width, self.height))
        if self.value != None:
            value_blit = self.font.render(str(self.value), True, (0, 0, 0))
            win.blit(value_blit, (self.start[0] + 10, self.start[1] + 10))
        return
...
class Display:
    def __init__(self, width, height):
        ...
        self.display_unit = DisplayUnit((550, 125), 100, 100)
    ...
    def show(self, cpu):
        ...
        self.show_ram(cpu)
        self.show_buses(cpu)
        self.display_unit.draw(self.win)
...
program_name = "divide.txt"  # the name of the program file
ram = RAM(16)
compiler = Compiler()
    ...
    if cpu.run and not pause:
        ...
        try:
            if isinstance(args, str) or isinstance(args, int):
                ...
                args = step_method(*args)
            if step == 9 and args != None:
                display.display_unit.update_value(args)
        except TypeError:
            args = step_method()
        step = next_step(step)
    display.update()
